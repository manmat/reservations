  ...
  Appointments2:{
   Auth:{
    Main:function(url)
    {
     ...
     formlet=Data.$(Data.$(Formlet1.Return(function(name1)
     {
      return function(p)
      {
       return Remoting.Send("Reservations:0",[name1,p]);
      };
     }),name),pw);
   ...
   MyPage:{
    ...
    select:function(page,r)
    {
     ...
     if(page.$==2)
      {
       matchValue=Remoting.Call("Reservations:1",[r]);
       if(matchValue.$==1)
        {
         x=matchValue.$0;
      ...
      {
       return page.$==1?Remoting.Call("Reservations:2",[r])?alert("Reserved"):alert("Available"):page.$==3?Remoting.Send("Reservations:4",[r]):page.$==4?Remoting.Send("Reservations:5",[r]):Remoting.Call("Reservations:3",[r])?alert("Success"):alert("Already reserved");
      }
    }
   }
  }
